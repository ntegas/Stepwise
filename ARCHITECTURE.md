...
## 29. Recommended Implementation Sequence — SUPERSEDED

**Superseded by `/ROADMAP.md`**, which is now the single authoritative implementation sequence (DEV-000 through DEV-037). This section is kept only as a historical record of the coarser five-spec sequence it replaces; do not treat it as current and do not follow it.

The one piece of concrete detail this section carried that wasn't yet written down elsewhere — the sync metadata column list (`id: UUID`, `updated_at`, `version`, `deleted_at`, `sync_status`) — has been relocated to `/DATA_MODEL.md` ("Sync metadata columns" section) before this section was retired, per `/ANTI_ERROR_STANDARD.md` §4.

<details>
<summary>Original text (historical, superseded)</summary>

...
- **Skill Graph / RPG mechanics — explicit guardrail**: do not introduce a Skill Graph, RPG-style levels, unlock trees, or any similar separate gamification system without a new, explicit product decision. §51's Skills concept remains a secondary, passively-derived analytical view over existing Activity/Goal data (`/DATA_MODEL.md` `skills`/`skill_sources`) — not a game system with its own progression state. No forward-looking architecture (schema, use cases, or UI scaffolding) for unlock/leveling mechanics is to be built ahead of such a decision.
